...
// Show_Registrations_Gallery.aspx page.  The Row or RecordControl classes are the 
...
namespace OLR.UI.Controls.Show_Registrations_Gallery
{
...
// Base class for the RegistrationsTableControlRow control on the Show_Registrations_Gallery page.
...
public class BaseRegistrationsTableControlRow : OLR.UI.BaseApplicationRecordControl
{
        ...
        {      
              ...
              // Register the event handlers.
                    ...
                    this.ValidationUid.Click += ValidationUid_Click;
        ...
        public virtual void SetValidationUid()
        {
            ...
            // Set the ValidationUid LinkButton on the webpage with value from the
            // DatabaseOLR_db%dbo.Registrations database record.

            // this.DataSource is the DatabaseOLR_db%dbo.Registrations record retrieved from the database.
            // this.ValidationUid is the ASP:LinkButton on the webpage.
            ...
            if (this.DataSource != null && this.DataSource.ValidationUidSpecified) {
               ...
               string formattedValue = this.DataSource.Format(RegistrationsTable.ValidationUid);
                ...
                this.ValidationUid.Text = formattedValue;
                  ...
                  this.ValidationUid.ToolTip = "Go to " + this.ValidationUid.Text.Replace("<wbr/>", "");
      ...
      // To customize, override this method in RegistrationsTableControlRow.
        ...
        public virtual void ValidationUid_Click(object sender, EventArgs args)
        {
            ...
            string url = @"../Registrations/Show-Registrations.aspx?Registrations={PK}";
...
#region "Helper Properties"
        ...
        public System.Web.UI.WebControls.LinkButton ValidationUid {
            get {
                return (System.Web.UI.WebControls.LinkButton)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "ValidationUid");
            }
        }
...
// Base class for the RegistrationsTableControl control on the Show_Registrations_Gallery page.
    ...
    public virtual void AddWarningMessageOnClick() {
        ...
        public virtual WhereClause CreateWhereClause()
        {
            ...
            if (MiscUtils.IsValueSelected(this.EventIdFilter)) {
                ...
                foreach (ListItem item in this.EventIdFilter.Items){
                    ...
                    if (item.Selected){
                        filter.iOR(RegistrationsTable.EventId, BaseFilter.ComparisonOperator.EqualsTo, item.Value, false, false);
                    }
                }
                wc.iAND(filter);
      ...
      String EventIdFilterSelectedValue = (String)HttpContext.Current.Session[HttpContext.Current.Session.SessionID + appRelativeVirtualPath + "EventIdFilter_Ajax"];
        ...
        if (EventIdFilterSelectedValue != null){
                        ...
                        foreach (string item in EventIdFilteritemListFromSession)
                        {
                            ...
                            {
                                filter.iOR(RegistrationsTable.EventId, BaseFilter.ComparisonOperator.EqualsTo, item, false, false);
                                index += 1;
                            }
                        }
                        wc.iAND(filter);
      ...
      RegistrationsRecord rec = new RegistrationsRecord();
                        ...
                        if (recControl.ValidationUid.Text != "") {
                            rec.Parse(recControl.ValidationUid.Text, RegistrationsTable.ValidationUid);
                  }
      ...
      this.ViewState["Vertical_Columns"] = this.VerticalColumns;
        ...
        public virtual void PDFButton_Click(object sender, ImageClickEventArgs args)
        {
            ...
            try {
                ...
                report.SpecificReportFileName = Page.Server.MapPath("Show-Registrations-Gallery.PDFButton.report");
                // report.Title replaces the value tag of page header and footer containing ${ReportTitle}
                report.Title = "Registrations";
                // If Show-Registrations-Gallery.PDFButton.report specifies a valid report template,
                     ...
                     if (records != null && records.Length > 0 && whereClause.RunQuery)
                    {
                    ...
                    
                        {
                            ...
                            // The 4th parameter represent the maximum length of the data value being shown
                                 ...
                                 report.AddData("${EventId}", "",ReportEnum.Align.Left, 300);
                             }else{
                                 ...
                                 if (_isExpandableNonCompositeForeignKey &&  ( _DFKA  != null)  &&  RegistrationsTable.EventId.IsApplyDisplayAs){
                                     report.AddData("${EventId}", _DFKA,ReportEnum.Align.Left, 300);
                                 }else{
                                     report.AddData("${EventId}", record.Format(RegistrationsTable.EventId), ReportEnum.Align.Left, 300);
                                 }
                             }
        ...
        public virtual void WordButton_Click(object sender, ImageClickEventArgs args)
        {
            ...
            try {
                ...
                report.SpecificReportFileName = Page.Server.MapPath("Show-Registrations-Gallery.WordButton.word");
                // report.Title replaces the value tag of page header and footer containing ${ReportTitle}
                report.Title = "Registrations";
                // If Show-Registrations-Gallery.WordButton.report specifies a valid report template,
                // AddColumn methods will generate a report template.
                // Each AddColumn method-call specifies a column
                // The 1st parameter represents the text of the column header
                // The 2nd parameter represents the horizontal alignment of the column header
...
#region "Helper Functions"
        ...
        public virtual RegistrationsTableControlRow[] GetSelectedRecordControls()
        {
            ...
            return (RegistrationsTableControlRow[])((new ArrayList()).ToArray(Type.GetType("OLR.UI.Controls.Show_Registrations_Gallery.RegistrationsTableControlRow")));
