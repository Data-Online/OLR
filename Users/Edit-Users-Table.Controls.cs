...
// Edit_Users_Table.aspx page.  The Row or RecordControl classes are the 
...
namespace OLR.UI.Controls.Edit_Users_Table
{
...
public class UsersTableControlRow : BaseUsersTableControlRow
{
...
public class UsersTableControl : BaseUsersTableControl
{
...
// Base class for the UsersTableControlRow control on the Edit_Users_Table page.
// Do not modify this class. Instead override any method in UsersTableControlRow.
public class BaseUsersTableControlRow : OLR.UI.BaseApplicationRecordControl
{
        public BaseUsersTableControlRow()
        {
        ...
        {      
              ...
              this.ActiveEventId.TextChanged += ActiveEventId_TextChanged;
        ...
        public virtual void SetActive()
        {
            ...
            // DatabaseOLR_db%dbo.Users database record.

            // this.DataSource is the DatabaseOLR_db%dbo.Users record retrieved from the database.
        ...
        public virtual void SetActiveEventId()
        {
            
            // If data was retrieved from UI previously, restore it
            if (this.PreviousUIData.ContainsKey(this.ActiveEventId.ID))
            {
            
                this.ActiveEventId.Text = this.PreviousUIData[this.ActiveEventId.ID].ToString();
              
                return;
            }
            
                    
            // Set the ActiveEventId TextBox on the webpage with value from the
            // DatabaseOLR_db%dbo.Users database record.

            // this.DataSource is the DatabaseOLR_db%dbo.Users record retrieved from the database.
            // this.ActiveEventId is the ASP:TextBox on the webpage.
                  
            if (this.DataSource != null && this.DataSource.ActiveEventIdSpecified) {
                								
                // If the ActiveEventId is non-NULL, then format the value.
                // The Format method will return the Display Foreign Key As (DFKA) value
               string formattedValue = this.DataSource.ActiveEventId.ToString();
                            
                this.ActiveEventId.Text = formattedValue;
                   
            } 
            
            else {
            
                // ActiveEventId is NULL in the database, so use the Default Value.  
                // Default Value could also be NULL.
        
              this.ActiveEventId.Text = UsersTable.ActiveEventId.DefaultValue;
        ...
        public virtual void SetPassword()
        {
            ...
            // If data was retrieved from UI previously, restore it
            if (this.PreviousUIData.ContainsKey(this.Password.ID))
            {
            
                this.Password.Text = this.PreviousUIData[this.Password.ID].ToString();
            ...
            if (this.DataSource != null && this.DataSource.PasswordSpecified) {
                ...
                // The Format method will use the Display Format
               string formattedValue = this.DataSource.Format(UsersTable.Password);
              ...
              this.Password.Text = UsersTable.Password.Format(UsersTable.Password.DefaultValue);
        ...
        public virtual void SetRoleId()
        {
            ...
            // Set the RoleId QuickSelector on the webpage with value from the
            // DatabaseOLR_db%dbo.Users database record.
            
            // this.DataSource is the DatabaseOLR_db%dbo.Users record retrieved from the database.
            // this.RoleId is the ASP:QuickSelector on the webpage.
            ...
            else
            {
                ...
                if (this.DataSource != null && this.DataSource.IsCreated)
                    ...
                    selectedValue = UsersTable.RoleId.DefaultValue;
                				
            }			
                
            // Add the Please Select item.
            if (selectedValue == null || selectedValue == "")
                  MiscUtils.ResetSelectedItem(this.RoleId, new ListItem(this.Page.GetResourceValue("Txt:PleaseSelect", "OLR"), "--PLEASE_SELECT--"));
                        
                  
            // Populate the item(s) to the control
            
            this.RoleId.SetFieldMaxLength(50);
                ...
                !MiscUtils.SetSelectedDisplayText(this.RoleId, selectedValue))
            {
                ...
                // construct a whereclause to query a record with DatabaseOLR_db%dbo.Roles.RoleID = selectedValue
                    
                CompoundFilter filter2 = new CompoundFilter(CompoundFilter.CompoundingOperators.And_Operator, null);
                WhereClause whereClause2 = new WhereClause();
                filter2.AddFilter(new BaseClasses.Data.ColumnValueFilter(Roles1Table.RoleID, selectedValue, BaseClasses.Data.BaseFilter.ComparisonOperator.EqualsTo, false));
                whereClause2.AddFilter(filter2, CompoundFilter.CompoundingOperators.And_Operator);

                // Execute the query
                try
                {
                    Roles1Record[] rc = Roles1Table.GetRecords(whereClause2, new OrderBy(false, false), 0, 1);
                    System.Collections.Generic.IDictionary<string, object> vars = new System.Collections.Generic.Dictionary<string, object> ();
                    // if find a record, add it to the dropdown and set it as selected item
                    if (rc != null && rc.Length == 1)
                    {
                        Roles1Record itemValue = rc[0];
                        string cvalue = null;
                        string fvalue = null;                        
                        if (itemValue.RoleIDSpecified)
                            cvalue = itemValue.RoleID.ToString(); 
                        Boolean _isExpandableNonCompositeForeignKey = UsersTable.Instance.TableDefinition.IsExpandableNonCompositeForeignKey(UsersTable.RoleId);
                        if(_isExpandableNonCompositeForeignKey && UsersTable.RoleId.IsApplyDisplayAs)
                            fvalue = UsersTable.GetDFKA(itemValue, UsersTable.RoleId);
                        if ((!_isExpandableNonCompositeForeignKey) || (String.IsNullOrEmpty(fvalue)))
                            fvalue = itemValue.Format(Roles1Table.RoleName);
              ...
              url += "?Target=" + this.RoleId.ClientID + "&DFKA=" + (this.Page as BaseApplicationPage).Encrypt("RoleName")+ "&IndexField=" + (this.Page as BaseApplicationPage).Encrypt("RoleID")+ "&EmptyValue=" + (this.Page as BaseApplicationPage).Encrypt("--PLEASE_SELECT--") + "&EmptyDisplayText=" + (this.Page as BaseApplicationPage).Encrypt(this.Page.GetResourceValue("Txt:PleaseSelect"))+ "&Mode=" + (this.Page as BaseApplicationPage).Encrypt("FieldValueSingleSelection") + "&RedirectStyle=" + (this.Page as BaseApplicationPage).Encrypt("Popup");
              
              this.RoleId.Attributes["onClick"] = "initializePopupPage(this, '" + url + "', " + Convert.ToString(RoleId.AutoPostBack).ToLower() + ", event); return false;";
        ...
        public virtual void SetUserName0()
        {
            ...
            // If data was retrieved from UI previously, restore it
            if (this.PreviousUIData.ContainsKey(this.UserName0.ID))
            {
            
                this.UserName0.Text = this.PreviousUIData[this.UserName0.ID].ToString();
        ...
        public virtual void SaveData()
        {
            ...
            if (this.DataSource.IsAnyValueChanged) {
                ...
                ((UsersTableControl)MiscUtils.GetParentControlObject(this, "UsersTableControl")).DataChanged = true;
                ((UsersTableControl)MiscUtils.GetParentControlObject(this, "UsersTableControl")).ResetData = true;
            }
        ...
        {	
            ...
            // save it into the Active field in DataSource DatabaseOLR_db%dbo.Users record.
        ...
        public virtual void GetActiveEventId()
        {
            
            // Retrieve the value entered by the user on the ActiveEventId ASP:TextBox, and
            // save it into the ActiveEventId field in DataSource DatabaseOLR_db%dbo.Users record.
            
            // Custom validation should be performed in Validate, not here.
                    
            // Save the value to data source
            this.DataSource.Parse(this.ActiveEventId.Text, UsersTable.ActiveEventId);							
        ...
        public virtual void GeteMail()
        {
            ...
            // save it into the eMail field in DataSource DatabaseOLR_db%dbo.Users record.
        ...
        public virtual void GetPassword()
        {
            ...
            // save it into the Password field in DataSource DatabaseOLR_db%dbo.Users record.
        ...
        {
         // Retrieve the value entered by the user on the RoleId ASP:QuickSelector, and
            // save it into the RoleId field in DataSource DatabaseOLR_db%dbo.Users record.
            
            // Custom validation should be performed in Validate, not here.
            
            this.DataSource.Parse(MiscUtils.GetValueSelectedPageRequest(this.RoleId), UsersTable.RoleId);			
        ...
        public virtual void GetUserName0()
        {
            ...
            // save it into the UserName field in DataSource DatabaseOLR_db%dbo.Users record.
            
            // Custom validation should be performed in Validate, not here.
                    
            // Save the value to data source
            this.DataSource.Parse(this.UserName0.Text, UsersTable.UserName0);							
      ...
      // To customize, override this method in UsersTableControlRow.
         ...
         
        {
            ...
            bool hasFiltersUsersTableControl = false;
            hasFiltersUsersTableControl = hasFiltersUsersTableControl && false; // suppress warning
        ...
        UsersTableControl tc= ((UsersTableControl)MiscUtils.GetParentControlObject(this, "UsersTableControl"));
                if (tc != null){
                    if (!this.IsNewRecord){
                        tc.AddToDeletedRecordIds((UsersTableControlRow)this);
                    }
                    this.Visible = false;
      ...
      this.Page.Response.Redirect(url);
        ...
        protected virtual void ActiveEventId_TextChanged(object sender, EventArgs args)
        {
        ...
        public String RecordUniqueId {
            get {
                return (string)this.ViewState["BaseUsersTableControlRow_Rec"];
            }
            set {
                this.ViewState["BaseUsersTableControlRow_Rec"] = value;
            }
        }
...
#region "Helper Properties"
        ...
        public System.Web.UI.WebControls.TextBox ActiveEventId {
            get {
                return (System.Web.UI.WebControls.TextBox)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "ActiveEventId");
            }
        }
        ...
        public BaseClasses.Web.UI.WebControls.QuickSelector RoleId {
            get {
                return (BaseClasses.Web.UI.WebControls.QuickSelector)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "RoleId");
            }
        }              
...
// Base class for the UsersTableControl control on the Edit_Users_Table page.
// Do not modify this class. Instead override any method in UsersTableControl.
public class BaseUsersTableControl : OLR.UI.BaseApplicationTableControl
{
         
       public BaseUsersTableControl()
        {
            this.Init += Control_Init;
            this.Load += Control_Load;
            this.PreRender += Control_PreRender;
        ...
        protected virtual void Control_Init(object sender, System.EventArgs e)
        {
           ...
           // Setup the filter and search.
            ...
            {
                string initialVal = "";
                if  (this.InSession(this.UserNameFilter)) 				
                    initialVal = this.GetFromSession(this.UserNameFilter);
                
                else
                    
                    initialVal = EvaluateFormula("URL(\"UserName\")");
                ...
                if (initialVal != null && initialVal != "")		
                {
                    ...
                    string[] UserNameFilteritemListFromSession = initialVal.Split(',');
                    int index = 0;
                    foreach (string item in UserNameFilteritemListFromSession)
                    {
                        ...
                        {
                            this.UserNameFilter.Items.Add(item);
                            this.UserNameFilter.Items[index].Selected = true;
                            index += 1;
                        }
                    }
                    foreach (ListItem listItem in this.UserNameFilter.Items)
                    {
    ...
    // Setup default pagination settings.
       ...
       // Setup the sorting events.
            ...
            this.SortControl.SelectedIndexChanged += new EventHandler(SortControl_SelectedIndexChanged);
              ...
              this.UserNameFilter.SelectedIndexChanged += UserNameFilter_SelectedIndexChanged;                  
        ...
        public virtual void LoadData()
        {
            ...
            try {
                ...
                if (this.AddNewRecord > 0) {
                    ...
                    foreach (UsersTableControlRow rc in this.GetRecordControls()) {
                        if (!rc.IsNewRecord) {
      ...
      public override void DataBind()
      {
        ...
        System.Web.UI.WebControls.Repeater rep = (System.Web.UI.WebControls.Repeater)(BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "UsersTableControlRepeater"));
        if (rep == null){return;}
        ...
        public void PreFetchForeignKeyValues() {
            ...
            this.Page.PregetDfkaRecords(UsersTable.ActiveEventId, this.DataSource);
            this.Page.PregetDfkaRecords(UsersTable.RoleId, this.DataSource);
        }
        ...
        public virtual void ResetControl()
        {
            ...
            this.UserNameFilter.ClearSelection();
        ...
        public virtual void SaveData()
        {
            ...
            foreach (UsersTableControlRow recCtl in this.GetRecordControls())
            {
      ...
      cols.Add(UsersTable.UserName0, true);
      
      cols.Add(UsersTable.RoleId, true);
      ...
      foreach(BaseColumn col in cols)
      {
          ...
          search.iOR(col, BaseFilter.ComparisonOperator.Contains, MiscUtils.GetSelectedValue(this.SearchText, this.GetFromSession(this.SearchText)), true, false);
            ...
            if (MiscUtils.IsValueSelected(this.UserNameFilter)) {
                        
                int selectedItemCount = 0;
                foreach (ListItem item in this.UserNameFilter.Items){
                    if (item.Selected) {
                        selectedItemCount += 1;
                ...
                foreach (ListItem item in this.UserNameFilter.Items){
                    ...
                    if (item.Selected){
                        filter.iOR(UsersTable.UserName0, BaseFilter.ComparisonOperator.EqualsTo, item.Value, false, false);
                    }
                }
                wc.iAND(filter);
      ...
      String UserNameFilterSelectedValue = (String)HttpContext.Current.Session[HttpContext.Current.Session.SessionID + appRelativeVirtualPath + "UserNameFilter_Ajax"];
            if (MiscUtils.IsValueSelected(UserNameFilterSelectedValue)) {

              
        if (UserNameFilterSelectedValue != null){
                        string[] UserNameFilteritemListFromSession = UserNameFilterSelectedValue.Split(',');
                        int index = 0;
                        WhereClause filter = new WhereClause();
                        foreach (string item in UserNameFilteritemListFromSession)
                        {
                            ...
                            {
                                filter.iOR(UsersTable.UserName0, BaseFilter.ComparisonOperator.EqualsTo, item, false, false);
                                index += 1;
                            }
                        }
                        wc.iAND(filter);
        ...
        public virtual string[] GetAutoCompletionList_SearchText(String prefixText,int count)
        {
            ...
            foreach (UsersRecord rec in recordList ){
                ...
                if (resultItem != null) {
                    ...
                    if(UsersTable.UserName0.IsFullTextSearchable) {
                        FullTextExpression ft = new FullTextExpression();
                        prText = ft.GetFirstNonExcludedTerm(prText);
                    }
                    if (!string.IsNullOrEmpty(prText) && resultItem.ToUpper(System.Threading.Thread.CurrentThread.CurrentCulture).Contains(prText.ToUpper(System.Threading.Thread.CurrentThread.CurrentCulture))) {
                        bool isAdded = FormatSuggestions(prText, resultItem, 50, "InMiddleOfMatchedString", "WordsStartingWithSearchString", "[^a-zA-Z0-9]", resultList, UsersTable.UserName0.IsFullTextSearchable);
                        if (isAdded) {
                            continue;
                        }
                    }
                ...
                resultItem = rec.Format(UsersTable.RoleId);
  
                if (resultItem != null) {
                    string prText = prefixText;
                    if(UsersTable.RoleId.IsFullTextSearchable) {
                        FullTextExpression ft = new FullTextExpression();
                        prText = ft.GetFirstNonExcludedTerm(prText);
                    }
                    if (!string.IsNullOrEmpty(prText) && resultItem.ToUpper(System.Threading.Thread.CurrentThread.CurrentCulture).Contains(prText.ToUpper(System.Threading.Thread.CurrentThread.CurrentCulture))) {
                        bool isAdded = FormatSuggestions(prText, resultItem, 50, "InMiddleOfMatchedString", "WordsStartingWithSearchString", "[^a-zA-Z0-9]", resultList, UsersTable.RoleId.IsFullTextSearchable);
                        if (isAdded) {
                            continue;
                        }
                    }
      ...
      UsersRecord rec = new UsersRecord();
                        ...
                        if (recControl.ActiveEventId.Text != "") {
                            rec.Parse(recControl.ActiveEventId.Text, UsersTable.ActiveEventId);
                  }
                        ...
                        if (MiscUtils.IsValueSelected(recControl.RoleId)) {
                            rec.Parse(recControl.RoleId.SelectedItem.Value, UsersTable.RoleId);
                        }
        ...
        public void AddToDeletedRecordIds(UsersTableControlRow rec)
        {
            if (rec.IsNewRecord) {
                return;
            }
        ...
        protected virtual bool InDeletedRecordIds(UsersTableControlRow rec)            
        {
            if (this.DeletedRecordIds == null || this.DeletedRecordIds.Length == 0) {
                return (false);
            }
        ...
        public virtual void SetUserNameLabel1()
                  {
        ...
        public virtual void SetUserNameFilter()
        {
            
            ArrayList UserNameFilterselectedFilterItemList = new ArrayList();
            string UserNameFilteritemsString = null;
            if (this.InSession(this.UserNameFilter))
                UserNameFilteritemsString = this.GetFromSession(this.UserNameFilter);
            
            if (UserNameFilteritemsString != null)
            {
                string[] UserNameFilteritemListFromSession = UserNameFilteritemsString.Split(',');
                foreach (string item in UserNameFilteritemListFromSession)
                {
                    UserNameFilterselectedFilterItemList.Add(item);
                }
            }
              
            			
            this.PopulateUserNameFilter(MiscUtils.GetSelectedValueList(this.UserNameFilter, UserNameFilterselectedFilterItemList), 500);
                    
              string url = this.ModifyRedirectUrl("../Users/Users-QuickSelector.aspx", "", true);
              
              url = this.Page.ModifyRedirectUrl(url, "", true);                                  
              
              url += "?Target=" + this.UserNameFilter.ClientID + "&IndexField=" + (this.Page as BaseApplicationPage).Encrypt("UserName")+ "&EmptyValue=" + (this.Page as BaseApplicationPage).Encrypt("--ANY--") + "&EmptyDisplayText=" + (this.Page as BaseApplicationPage).Encrypt(this.Page.GetResourceValue("Txt:All")) + "&RedirectStyle=" + (this.Page as BaseApplicationPage).Encrypt("Popup");
              
              this.UserNameFilter.Attributes["onClick"] = "initializePopupPage(this, '" + url + "', " + Convert.ToString(UserNameFilter.AutoPostBack).ToLower() + ", event); return false;";
                    ...
                    
        {
              ...
              // 1. Setup the static list items
                ...
                this.SortControl.Items.Add(new ListItem(this.Page.ExpandResourceValue("Last Login Date {Txt:Ascending}"), "LastLoginDate Asc"));
              
                this.SortControl.Items.Add(new ListItem(this.Page.ExpandResourceValue("Last Login Date {Txt:Descending}"), "LastLoginDate Desc"));
              
                this.SortControl.Items.Add(new ListItem(this.Page.ExpandResourceValue("Role {Txt:Ascending}"), "RoleId Asc"));
              
                this.SortControl.Items.Add(new ListItem(this.Page.ExpandResourceValue("Role {Txt:Descending}"), "RoleId Desc"));
              
                this.SortControl.Items.Add(new ListItem(this.Page.ExpandResourceValue("User Name {Txt:Ascending}"), "UserName Asc"));
              
                this.SortControl.Items.Add(new ListItem(this.Page.ExpandResourceValue("User Name {Txt:Descending}"), "UserName Desc"));
              
                this.SortControl.Items.Add(new ListItem(this.Page.ExpandResourceValue("Active Event {Txt:Ascending}"), "ActiveEventId Asc"));
              
                this.SortControl.Items.Add(new ListItem(this.Page.ExpandResourceValue("Active Event {Txt:Descending}"), "ActiveEventId Desc"));
        ...
        protected virtual void PopulateUserNameFilter(ArrayList selectedValue, int maxItems)
                    
        {
            ...
            WhereClause wc = this.CreateWhereClause_UserNameFilter();            
            this.UserNameFilter.Items.Clear();
            			  			
            // Set up the WHERE and the ORDER BY clause by calling the CreateWhereClause_UserNameFilter function.
            ...
            if (wc.RunQuery)
            {
                ...
                values = UsersTable.GetValues(UsersTable.UserName0, wc, orderBy, maxItems);
            ...
            string fvalue;
            if ( UsersTable.UserName0.IsColumnValueTypeBoolean()) {
                    fvalue = cvalue;
                }else {
                    fvalue = UsersTable.UserName0.Format(cvalue);
                }
                if (fvalue == null) {
                    fvalue = "";
                }
                ...
                ListItem dupItem = this.UserNameFilter.Items.FindByText(fvalue);
                ...
                this.UserNameFilter.Items.Add(newItem);
            ...
            this.UserNameFilter.SetFieldMaxLength(50);
                                 
                  
            // Add the selected value.
            if (this.UserNameFilter.Items.Count == 0)
                this.UserNameFilter.Items.Add(new ListItem(Page.GetResourceValue("Txt:All", "OLR"), "--ANY--"));
        ...
        public virtual WhereClause CreateWhereClause_UserNameFilter()
        {
            // Create a where clause for the filter UserNameFilter.
            // This function is called by the Populate method to load the items 
            // in the UserNameFilterQuickSelector
            ...
            UserNameFilterselectedFilterItemList = MiscUtils.GetSelectedValueList(this.UserNameFilter, UserNameFilterselectedFilterItemList); 
            WhereClause wc = new WhereClause();
            if (UserNameFilterselectedFilterItemList == null || UserNameFilterselectedFilterItemList.Count == 0)
                wc.RunQuery = false;
            else            
            {
                foreach (string item in UserNameFilterselectedFilterItemList)
                {
            
      
   
                    wc.iOR(UsersTable.UserName0, BaseFilter.ComparisonOperator.EqualsTo, item);
        ...
        protected virtual void Control_PreRender(object sender, System.EventArgs e)
        {
            ...
            {
            System.Web.UI.WebControls.Repeater rep = (System.Web.UI.WebControls.Repeater)this.FindControl("UsersTableControlRepeater");
        ...
        protected override void SaveControlsToSession()
        {
            ...
            ArrayList UserNameFilterselectedFilterItemList = MiscUtils.GetSelectedValueList(this.UserNameFilter, null);
            string UserNameFilterSessionString = "";
            if (UserNameFilterselectedFilterItemList != null){
                foreach (string item in UserNameFilterselectedFilterItemList){
                    UserNameFilterSessionString = String.Concat(UserNameFilterSessionString ,"," , item);
                }
            }
            this.SaveToSession(this.UserNameFilter, UserNameFilterSessionString);
      ...
      this.SaveToSession("SearchText_Ajax", this.SearchText.Text);
            ...
            this.SaveToSession("UserNameFilter_Ajax", UserNameFilterSessionString);
        ...
        protected override void ClearControlsFromSession()
        {
            ...
            this.RemoveFromSession(this.UserNameFilter);
    ...
    this.RemoveFromSession(this, "Page_Size");
        ...
        protected override void LoadViewState(object savedState)
        {
            ...
            string orderByStr = (string)ViewState["UsersTableControl_OrderBy"];
        ...
        {            
            ...
            if (this.CurrentSortOrder != null) {
                this.ViewState["UsersTableControl_OrderBy"] = this.CurrentSortOrder.ToXmlString();
            }
    ...
    this.ViewState["Page_Size"] = this.PageSize;
              ...
              
        {
         ...
         themeButtonFiltersButton.ArrowImage.ImageUrl = "../Images/ButtonExpandArrow.png";
            ...
            if (MiscUtils.IsValueSelected(UserNameFilter))
        ...
        protected virtual void SortControl_SelectedIndexChanged(object sender, EventArgs args)
        {
                  ...
                  foreach (BaseClasses.Data.BaseColumn ColumnNam in UsersTable.GetColumns())
                  {
                ...
                OrderByItem sd = this.CurrentSortOrder.Find(UsersTable.GetColumnByName(SelVal1));
                if (sd == null || this.CurrentSortOrder.Items != null)
                {
                // First time sort, so add sort order for Discontinued.
                if (UsersTable.GetColumnByName(SelVal1) != null)
                {
                  this.CurrentSortOrder.Reset();
                }
                ...
                if ((this.CurrentSortOrder).GetType() == typeof(GeoOrderBy)) this.CurrentSortOrder = new OrderBy(true, false);
                  ...
                  if (SelVal1 != "--PLEASE_SELECT--" && UsersTable.GetColumnByName(SelVal1) != null)
                  {
                    if (words1[words1.Length - 1].Contains("ASC"))
                  {
                  this.CurrentSortOrder.Add(UsersTable.GetColumnByName(SelVal1),OrderByItem.OrderDir.Asc);
                    }
                    else
                    {
                      if (words1[words1.Length - 1].Contains("DESC"))
                  {
                  this.CurrentSortOrder.Add(UsersTable.GetColumnByName(SelVal1),OrderByItem.OrderDir.Desc );
                      }
                    }
                  }
        ...
        protected virtual void UserNameFilter_SelectedIndexChanged(object sender, EventArgs args)
        {
            // Setting the DataChanged to True results in the page being refreshed with
            // the most recent data from the database.  This happens in PreRender event
            // based on the current sort, search and filter criteria.
...
#region "Helper Properties"
        ...
        public BaseClasses.Web.UI.WebControls.QuickSelector UserNameFilter {
            get {
                return (BaseClasses.Web.UI.WebControls.QuickSelector)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "UserNameFilter");
            }
        }              
        
        public System.Web.UI.WebControls.Literal UserNameLabel1 {
            get {
                return (System.Web.UI.WebControls.Literal)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "UserNameLabel1");
            }
        }
...
#region "Helper Functions"
        ...
        public override string EvaluateExpressions(string url, string arg, bool bEncrypt)
        {
            ...
            if (needToProcess) {
                UsersTableControlRow recCtl = this.GetSelectedRecordControl();
        ...
        {
            int counter = 0;
            foreach (UsersTableControlRow recControl in this.GetRecordControls())
            {
        ...
        public virtual UsersTableControlRow GetSelectedRecordControl()
        {
        UsersTableControlRow[] selectedList = this.GetSelectedRecordControls();
        ...
        public virtual UsersTableControlRow[] GetSelectedRecordControls()
        {
            ...
            return (UsersTableControlRow[])(selectedList.ToArray(Type.GetType("OLR.UI.Controls.Edit_Users_Table.UsersTableControlRow")));
          
        }

        public virtual void DeleteSelectedRecords(bool deferDeletion)
        {
            UsersTableControlRow[] recordList = this.GetSelectedRecordControls();
            if (recordList.Length == 0) {
                // Localization.
                throw new Exception(Page.GetResourceValue("Err:NoRecSelected", "OLR"));
            }
            
            foreach (UsersTableControlRow recCtl in recordList)
            {
                if (deferDeletion) {
        ...
        public virtual UsersTableControlRow[] GetRecordControls()
        {
            Control[] recCtrls = BaseClasses.Utils.MiscUtils.FindControlsRecursively(this, "UsersTableControlRow");
	          List<UsersTableControlRow> list = new List<UsersTableControlRow>();
	          foreach (UsersTableControlRow recCtrl in recCtrls) {
		          list.Add(recCtrl);
	          }
	          return list.ToArray();
        }
