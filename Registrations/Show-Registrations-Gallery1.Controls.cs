...
// Show_Registrations_Gallery1.aspx page.  The Row or RecordControl classes are the 
...
namespace OLR.UI.Controls.Show_Registrations_Gallery1
{
...
// Base class for the RegistrationsTableControlRow control on the Show_Registrations_Gallery1 page.
        ...
        public virtual void SetImagewithOverlaidText()
                  {
                        ...
                        this.ImagewithOverlaidText.Text = EvaluateFormula("RegistrationsTableControlRow.GetImageWithOverlaidText(\"\", \"<a class=\\\"button_link\\\" href=\\\"\" + RegistrationsTableControlRow.ModifyRedirectUrl(\"../Registrations/Show-Registrations.aspx?Registrations={PK}\", \"\", True) + \"\\\">\" + ValidationUid + \"</a>\", RegistrationsTableControlRow.ModifyRedirectUrl(\"../Registrations/Show-Registrations.aspx?Registrations={PK}\", \"\", True))");
...
// Base class for the RegistrationsTableControl control on the Show_Registrations_Gallery1 page.
...
public class BaseRegistrationsTableControl : OLR.UI.BaseApplicationTableControl
{
       ...
       public BaseRegistrationsTableControl()
        ...
        protected virtual void Control_Init(object sender, System.EventArgs e)
        {
           ...
           // Setup the filter and search.
            ...
            {
                string initialVal = "";
                if  (this.InSession(this.ValidationUidFilter)) 				
                    initialVal = this.GetFromSession(this.ValidationUidFilter);
                
                else
                    
                    initialVal = EvaluateFormula("URL(\"ValidationUid\")");
                ...
                if (initialVal != null && initialVal != "")		
                {
                    ...
                    string[] ValidationUidFilteritemListFromSession = initialVal.Split(',');
                    int index = 0;
                    foreach (string item in ValidationUidFilteritemListFromSession)
                    {
                        ...
                        {
                            this.ValidationUidFilter.Items.Add(item);
                            this.ValidationUidFilter.Items[index].Selected = true;
                            index += 1;
                        }
                    }
                    foreach (ListItem listItem in this.ValidationUidFilter.Items)
                    {
    ...
    // Setup default pagination settings.
       ...
       // Setup the sorting events.
            ...
            this.SortControl.SelectedIndexChanged += new EventHandler(SortControl_SelectedIndexChanged);
              ...
              this.ValidationUidFilter.SelectedIndexChanged += ValidationUidFilter_SelectedIndexChanged;                  
        ...
        {    
                  ...
                  
            {
                ...
                return RegistrationsTable.GetRecords(join, where, orderBy, this.PageIndex, (this.PageSize * this.VerticalColumns));
      ...
      public virtual RegistrationsRecord[] CopyArray(int index, RegistrationsRecord[,] recs, int columns)
      {
        RegistrationsRecord[] recsCopy = new RegistrationsRecord[columns];
      ...
      public override void DataBind()
      {
          ...
          //  LoadData for DataSource for chart and report if they exist
            ...
            RegistrationsRecord[,] recs = new RegistrationsRecord[Convert.ToInt32(rows - 1) + 1, this.VerticalColumns];
            ...
            RegistrationsRecord[] HColDataSource = new RegistrationsRecord[Convert.ToInt32(rows)];
            ...
            System.Web.UI.WebControls.Repeater rep1 = (System.Web.UI.WebControls.Repeater)BaseClasses.Utils.MiscUtils.FindControlRecursively(rep.Items[PageIndex], "RegistrationsTableControlRepeater1");
            ...
            foreach (System.Web.UI.WebControls.RepeaterItem repItem in rep1.Items)
            {
              ...
              RegistrationsRecord[] VColDataSource = new RegistrationsRecord[this.VerticalColumns];

              System.Web.UI.WebControls.Repeater repControl = (System.Web.UI.WebControls.Repeater)repItem.FindControl("RegistrationsTableControlCellRepeater");
              if (repControl == null)
                  break;
              repControl.DataSource = VColDataSource;
              repControl.DataBind();

              RegistrationsRecord[] recsCopy = CopyArray(index, recs, this.VerticalColumns);
              int indexInnerRep = 0;
              foreach (System.Web.UI.WebControls.RepeaterItem innerRepItem in repControl.Items) {
                RegistrationsTableControlRow recControl = (RegistrationsTableControlRow)innerRepItem.FindControl("RegistrationsTableControlRow");
            ...
            // Call the Set methods for each controls on the panel
                ...
                SetValidationUidFilter();
                SetValidationUidLabel1();
    ...
    public virtual void AddWarningMessageOnClick() {
        ...
        public virtual void ResetControl()
        {
            ...
            this.ValidationUidFilter.ClearSelection();
        ...
        public virtual WhereClause CreateWhereClause()
        {
            ...
            if (MiscUtils.IsValueSelected(this.ValidationUidFilter)) {
                        
                int selectedItemCount = 0;
                foreach (ListItem item in this.ValidationUidFilter.Items){
                    if (item.Selected) {
                        selectedItemCount += 1;
                ...
                foreach (ListItem item in this.ValidationUidFilter.Items){
                    ...
                    if (item.Selected){
                        filter.iOR(RegistrationsTable.ValidationUid, BaseFilter.ComparisonOperator.EqualsTo, item.Value, false, false);
                    }
                }
                wc.iAND(filter);
      ...
      String ValidationUidFilterSelectedValue = (String)HttpContext.Current.Session[HttpContext.Current.Session.SessionID + appRelativeVirtualPath + "ValidationUidFilter_Ajax"];
            if (MiscUtils.IsValueSelected(ValidationUidFilterSelectedValue)) {

              
        if (ValidationUidFilterSelectedValue != null){
                        string[] ValidationUidFilteritemListFromSession = ValidationUidFilterSelectedValue.Split(',');
                        int index = 0;
                        WhereClause filter = new WhereClause();
                        foreach (string item in ValidationUidFilteritemListFromSession)
                        {
                            ...
                            {
                                filter.iOR(RegistrationsTable.ValidationUid, BaseFilter.ComparisonOperator.EqualsTo, item, false, false);
                                index += 1;
                            }
                        }
                        wc.iAND(filter);
        ...
        protected virtual void AddNewRecords()
        {
            ...
            if (!this.ResetData)
            {
              ...
              RegistrationsTableControlRow[] recControls = this.GetRecordControls();
                foreach (RegistrationsTableControlRow recControl in recControls) 
		{
      ...
      RegistrationsRecord rec = new RegistrationsRecord();
        ...
        public virtual void SetValidationUidLabel1()
                  {
        ...
        public virtual void SetValidationUidFilter()
        {
            
            ArrayList ValidationUidFilterselectedFilterItemList = new ArrayList();
            string ValidationUidFilteritemsString = null;
            if (this.InSession(this.ValidationUidFilter))
                ValidationUidFilteritemsString = this.GetFromSession(this.ValidationUidFilter);
            
            if (ValidationUidFilteritemsString != null)
            {
                string[] ValidationUidFilteritemListFromSession = ValidationUidFilteritemsString.Split(',');
                foreach (string item in ValidationUidFilteritemListFromSession)
                {
                    ValidationUidFilterselectedFilterItemList.Add(item);
                }
            }
              
            			
            this.PopulateValidationUidFilter(MiscUtils.GetSelectedValueList(this.ValidationUidFilter, ValidationUidFilterselectedFilterItemList), 500);
                    
              string url = this.ModifyRedirectUrl("../Registrations/Registrations-QuickSelector.aspx", "", true);
              
              url = this.Page.ModifyRedirectUrl(url, "", true);                                  
              
              url += "?Target=" + this.ValidationUidFilter.ClientID + "&IndexField=" + (this.Page as BaseApplicationPage).Encrypt("ValidationUid")+ "&EmptyValue=" + (this.Page as BaseApplicationPage).Encrypt("--ANY--") + "&EmptyDisplayText=" + (this.Page as BaseApplicationPage).Encrypt(this.Page.GetResourceValue("Txt:All")) + "&RedirectStyle=" + (this.Page as BaseApplicationPage).Encrypt("Popup");
              
              this.ValidationUidFilter.Attributes["onClick"] = "initializePopupPage(this, '" + url + "', " + Convert.ToString(ValidationUidFilter.AutoPostBack).ToLower() + ", event); return false;";
                    ...
                    
        {
              ...
              // 1. Setup the static list items
                ...
                this.SortControl.Items.Add(new ListItem(this.Page.ExpandResourceValue("Special Requirements {Txt:Ascending}"), "SpecialRequirements Asc"));
              
                this.SortControl.Items.Add(new ListItem(this.Page.ExpandResourceValue("Special Requirements {Txt:Descending}"), "SpecialRequirements Desc"));
        ...
        // Get the filters' data for ValidationUidFilter.
                
        protected virtual void PopulateValidationUidFilter(ArrayList selectedValue, int maxItems)
                    
        {
            ...
            WhereClause wc = this.CreateWhereClause_ValidationUidFilter();            
            this.ValidationUidFilter.Items.Clear();
            			  			
            // Set up the WHERE and the ORDER BY clause by calling the CreateWhereClause_ValidationUidFilter function.
            // It is better to customize the where clause there.
             
            
            
            OrderBy orderBy = new OrderBy(false, false);
            orderBy.Add(RegistrationsTable.ValidationUid, OrderByItem.OrderDir.Asc);                
            ...
            if (wc.RunQuery)
            {
                ...
                values = RegistrationsTable.GetValues(RegistrationsTable.ValidationUid, wc, orderBy, maxItems);
            ...
            string fvalue;
            if ( RegistrationsTable.ValidationUid.IsColumnValueTypeBoolean()) {
                    fvalue = cvalue;
                }else {
                    fvalue = RegistrationsTable.ValidationUid.Format(cvalue);
                }
                if (fvalue == null) {
                    fvalue = "";
                }
                ...
                ListItem dupItem = this.ValidationUidFilter.Items.FindByText(fvalue);
                ...
                this.ValidationUidFilter.Items.Add(newItem);
            ...
            this.ValidationUidFilter.SetFieldMaxLength(50);
                                 
                  
            // Add the selected value.
            if (this.ValidationUidFilter.Items.Count == 0)
                this.ValidationUidFilter.Items.Add(new ListItem(Page.GetResourceValue("Txt:All", "OLR"), "--ANY--"));
        ...
        public virtual WhereClause CreateWhereClause_ValidationUidFilter()
        {
            // Create a where clause for the filter ValidationUidFilter.
            // This function is called by the Populate method to load the items 
            // in the ValidationUidFilterQuickSelector
            ...
            ValidationUidFilterselectedFilterItemList = MiscUtils.GetSelectedValueList(this.ValidationUidFilter, ValidationUidFilterselectedFilterItemList); 
            WhereClause wc = new WhereClause();
            if (ValidationUidFilterselectedFilterItemList == null || ValidationUidFilterselectedFilterItemList.Count == 0)
                wc.RunQuery = false;
            else            
            {
                foreach (string item in ValidationUidFilterselectedFilterItemList)
                {
            
      
   
                    wc.iOR(RegistrationsTable.ValidationUid, BaseFilter.ComparisonOperator.EqualsTo, item);
        ...
        protected override void SaveControlsToSession()
        {
            ...
            ArrayList ValidationUidFilterselectedFilterItemList = MiscUtils.GetSelectedValueList(this.ValidationUidFilter, null);
            string ValidationUidFilterSessionString = "";
            if (ValidationUidFilterselectedFilterItemList != null){
                foreach (string item in ValidationUidFilterselectedFilterItemList){
                    ValidationUidFilterSessionString = String.Concat(ValidationUidFilterSessionString ,"," , item);
                }
            }
            this.SaveToSession(this.ValidationUidFilter, ValidationUidFilterSessionString);
        ...
        protected  void SaveControlsToSession_Ajax()
        {
            ...
            this.SaveToSession("ValidationUidFilter_Ajax", ValidationUidFilterSessionString);
        ...
        protected override void ClearControlsFromSession()
        {
            ...
            this.RemoveFromSession(this.ValidationUidFilter);
    ...
    this.ViewState["Page_Size"] = this.PageSize;
      ...
      this.ViewState["Vertical_Columns"] = this.VerticalColumns;
              ...
              
        {
                    ...
                    this.ImportButton.PostBackUrl = "../Shared/SelectFileToImport.aspx?TableName=Registrations" ;
              ...
              
        {
              ...
              try
              {
                    string url = "../Registrations/Add-Registrations.aspx?TabVisible=False&SaveAndNewVisible=False";
              ...
              
        {
         ...
         themeButtonFiltersButton.ArrowImage.ImageUrl = "../Images/ButtonExpandArrow.png";
            ...
            if (MiscUtils.IsValueSelected(ValidationUidFilter))
        ...
        public virtual void PDFButton_Click(object sender, ImageClickEventArgs args)
        {
            ...
            try {
                ...
                report.SpecificReportFileName = Page.Server.MapPath("Show-Registrations-Gallery1.PDFButton.report");
                // report.Title replaces the value tag of page header and footer containing ${ReportTitle}
                report.Title = "Registrations";
                // If Show-Registrations-Gallery1.PDFButton.report specifies a valid report template,
        ...
        public virtual void WordButton_Click(object sender, ImageClickEventArgs args)
        {
            ...
            try {
                ...
                report.SpecificReportFileName = Page.Server.MapPath("Show-Registrations-Gallery1.WordButton.word");
                // report.Title replaces the value tag of page header and footer containing ${ReportTitle}
                report.Title = "Registrations";
                // If Show-Registrations-Gallery1.WordButton.report specifies a valid report template,
                // AddColumn methods will generate a report template.
                // Each AddColumn method-call specifies a column
                // The 1st parameter represents the text of the column header
                // The 2nd parameter represents the horizontal alignment of the column header
        ...
        protected virtual void ValidationUidFilter_SelectedIndexChanged(object sender, EventArgs args)
        {
            // Setting the DataChanged to True results in the page being refreshed with
            // the most recent data from the database.  This happens in PreRender event
            // based on the current sort, search and filter criteria.
...
#region "Helper Properties"
        ...
        public BaseClasses.Web.UI.WebControls.QuickSelector ValidationUidFilter {
            get {
                return (BaseClasses.Web.UI.WebControls.QuickSelector)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "ValidationUidFilter");
            }
        }              
        
        public System.Web.UI.WebControls.Literal ValidationUidLabel1 {
            get {
                return (System.Web.UI.WebControls.Literal)BaseClasses.Utils.MiscUtils.FindControlRecursively(this, "ValidationUidLabel1");
            }
        }
...
#region "Helper Functions"
        ...
        public virtual RegistrationsTableControlRow[] GetSelectedRecordControls()
        {
            ...
            return (RegistrationsTableControlRow[])((new ArrayList()).ToArray(Type.GetType("OLR.UI.Controls.Show_Registrations_Gallery1.RegistrationsTableControlRow")));
